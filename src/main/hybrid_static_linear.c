...
int main(int argsc, char* argsv[])
{
	...
	REAL minRowSliceLength = options.rowSliceLength;

	// Unordered runs
	// --------------------------------------------------------------------------------------------------

#ifdef ARCH_MIC_OFFLOAD
	#pragma offload target(mic:3) in(quintets[0:nnz], nnz, rowCount, colCount, numBlocks, numThreads, threadsPerBlock, minRowSliceLength)
#endif
	{
...
//			strcat(orderingHeaders, "un;");
		...
		partitioning_1DStaticRowSliceLinear_CSR(rowCount, colCount, SIMD_LENGTH, &subMtxList);
		// partitioning_1DRowSliceLinear_CSR(spmCsr, targetedCacheSizeInKB, minRowSliceLength, minRowSliceLength, &subMtxList, partitioning_calculateSizeInKBDefault);
		...
		// ----------------------------------------------------------------------------------------------------
		{
			...
			// ------------------------------------------------------------------------------------------------
			{
				...
				spmxv_static_prepFromSubMtxList_rowWise_hybrid_JDS_CSR(
						spmCsrCounterpart, subMtxList, numThreads, permutation, &jobBatchArrPerThread, &jobBatchCountPerThread);
...
/*
			...
			// ------------------------------------------------------------------------------------------------
			{
				...
				job_batch_printArrMultiple("batches;", denseJobBatchArrPerThread, denseJobBatchCountPerThread, numThreads);
...
*/
			...
			// ----------------------------------------------------------------------------------------------------------------------------
			{
				...
				spmxv_dws_prepFromSubMtxList_colNet_hybrid_JDS_CSR(
						spmCsrCounterpart, permutation, subMtxList, numThreads, options.stealTreshold, SCHEDULER_RING, &blockInfos);
				...
				// Clean up dynamic scheduling data structures
				spmxv_dws_cleanUp(blockInfos, numThreads, SCHEDULER_RING);
				lg_deleteShallowMultiple(execHistoryPerThread, numThreads);
			...
			// ----------------------------------------------------------------------------------------------------------------------------
			{
				...
				block_info_t** blockInfos = NULL;
				spmxv_dws_prepFromSubMtxList_colNet_hybrid_JDS_CSR(
						spmCsrCounterpart, permutation, subMtxList, numBlocks, options.stealTreshold, SCHEDULER_RING, &blockInfos);
				...
				// Clean up dynamic scheduling data structures
				spmxv_dws_cleanUp(blockInfos, numBlocks, SCHEDULER_RING);
				lg_deleteShallowMultiple(execHistoryPerThread, numThreads);
				lg_deleteShallowMultiple(execHistoryPerBlock, numBlocks);
				...
				vector_real_compare("DWS_RING_SHARED_BLOCK_WARMUP", ySequentialCSR, yDwsRingScatterSharedBlock);
...
//		vector_real_compare("OMP_TASK", comparisonVector, yOmpTask);
		...
		PRINTF("%d;%d;%d;%0.0f;%0.1f;%0.0f;%0.0f;%0.1f;%0.1f;%0.1f;%d;",
				...
				columnMinNNZ, columnAvgNNZ, columnMaxNNZ, minRowSliceLength,
